...
export default function HomePage() {
  ...
  useEffect(() => {
    const handleOnline = () => {
      ...
      alert(
        "Network connection restored. You can now try blockchain operations again."
      );
    };
    ...
    const handleOffline = () => {
      ...
      // Show a temporary notification
      setErrorMessage(
        "Network connection lost. Using local storage until connection is restored."
      );
    };
    ...
    if (connected && publicKey) {
      ...
      const fetchUsername = async () => {
        try {
          ...
          const isFallback = await lightService.isUsingFallbackStorage(
            publicKey
          );
          ...
          setErrorMessage(
            "Failed to load username: " + (error.message || "Unknown error")
          );
          setShowUsernameModal(true);
        } finally {
          setIsLoading(false);
  ...
  const handleSaveUsername = useCallback(async () => {
    ...
    if (username.length > config.MAX_USERNAME_LENGTH) {
      setErrorMessage(
        `Username cannot be longer than ${config.MAX_USERNAME_LENGTH} characters`
      );
      return;
    }
    ...
    try {
      ...
      if (result.metadata.fallback) {
        ...
        setLoadingMessage("");
        alert(
          `Username @${username} successfully registered on the blockchain!`
        );
      }
    } catch (error) {
  ...
  const handleRetryBlockchainRegistration = async () => {
    ...
    try {
      ...
      // Create a compressed NFT for the username
      const result = await lightService.createUsernameNFT(
        publicKey,
        savedUsername
      );
      ...
      if (!result.metadata.fallback) {
        ...
        setErrorMessage(
          "Blockchain registration failed again. Please try later when network conditions improve."
        );
      }
    } catch (error) {
  ...
  const handleUsernameRestored = (restoredUsername) => {
    ...
    lightService
      .isUsingFallbackStorage(publicKey)
      .then(setUsingFallbackStorage);
  };
  ...
  const executeSendTransaction = async () => {
    ...
    try {
      ...
      const usernameExists =
        recipientUsername.length > 2 && recipientUsername !== "notfound";
      ...
      <div className="flex items-center justify-between mb-8 border-b pb-4">
        ...
        <div className="flex items-center gap-2">
          {!isOnline && (
            <Badge
              variant="outline"
              className="text-xs font-normal border-red-200 bg-red-50 text-red-600"
            >
              Offline
            </Badge>
          )}
          <Badge
            variant="outline"
            className="text-xs font-normal border-gray-200 text-gray-600"
          >
            devnet
          </Badge>
        </div>
        ...
        <div className="text-center mb-8">
          ...
          <div className="text-sm text-gray-500 flex items-center justify-center">
            ...
            <Button
              ...
              onClick={() => {
                const textToCopy = savedUsername
                  ? `@${savedUsername}`
                  : publicKey.toString();
        ...
        <div className="text-center py-8 mb-8">
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-xs text-gray-500 max-w-xs text-center">
              Connect with Phantom, Solflare or other Solana wallets to
              continue. This will be your identity for username registration.
            </p>
          </div>
        </div>
      )}
